...
const FEATURED_BOOK_CONFIG = [
  { projectTitle: 'Henry and Husky', chapterNumber: 1, sceneNumber: 1, genre: 'Adventure' },
  { projectTitle: 'On Ice and Lanes', chapterNumber: 4, sceneNumber: null, genre: 'Fiction' },
  { projectTitle: 'Blind Target', chapterNumber: 2, sceneNumber: null, genre: 'Thriller' },
];

let featuredBooksCache: { data: any; cachedAt: number } | null = null;
const FEATURED_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

app.get('/api/featured-books', async (_req, res) => {
  // Serve from cache if fresh
  if (featuredBooksCache && Date.now() - featuredBooksCache.cachedAt < FEATURED_CACHE_TTL) {
    return res.json(featuredBooksCache.data);
  }
  try {
    const results = [];
    for (const cfg of FEATURED_BOOK_CONFIG) {
      const [project] = await db.select().from(projects)
        .where(eq(projects.title, cfg.projectTitle))
        .limit(1);
      if (!project) continue;

      const [chapter] = await db.select().from(chapters)
        .where(and(eq(chapters.projectId, project.id), eq(chapters.number, cfg.chapterNumber)))
        .limit(1);
      if (!chapter) continue;

      // Find audio — scene-level if sceneNumber specified, otherwise first active for chapter
      let audioUrl: string | null = null;
      if (cfg.sceneNumber != null) {
        const scenes = (chapter.scenes as any[]) || [];
        const targetScene = scenes.sort((a: any, b: any) => (a.order || 0) - (b.order || 0))[cfg.sceneNumber - 1];
        if (targetScene) {
          const [sceneAudio] = await db.select().from(audioGenerations)
            .where(and(
              eq(audioGenerations.chapterId, chapter.id),
              eq(audioGenerations.sceneId, targetScene.id),
              eq(audioGenerations.isActive, true),
            ))
            .orderBy(desc(audioGenerations.createdAt))
            .limit(1);
      ...
      // Fallback: any active audio for this chapter
      if (!audioUrl) {
        const [chapterAudio] = await db.select().from(audioGenerations)
          .where(and(eq(audioGenerations.chapterId, chapter.id), eq(audioGenerations.isActive, true)))
          .orderBy(desc(audioGenerations.createdAt))
          .limit(1);
        audioUrl = chapterAudio?.audioUrl || null;
      }
      ...
      results.push({
        ...
        audioUrl,
        audioLabel: cfg.sceneNumber != null
          ? `Chapter ${cfg.chapterNumber} · Scene ${cfg.sceneNumber}`
          : `Chapter ${cfg.chapterNumber}`,
        chapterTitle: chapter.title || `Chapter ${cfg.chapterNumber}`,
        genre: cfg.genre,
      });
    }
    featuredBooksCache = { data: results, cachedAt: Date.now() };
    res.json(results);
  } catch (e: any) {
