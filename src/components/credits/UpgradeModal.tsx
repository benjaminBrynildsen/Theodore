...
// A/B test for the audio_cap anchor: 'stacked' shows three subscription
// benchmarks (ChatGPT/Audible/Spotify), 'audible' shows the single Audible
// consumer→creator flip. Stable per-visitor via localStorage so the same
...
export function UpgradeModal() {
  ...
  useEffect(() => {
    ...
    const extra = isAudioCap ? { anchor_variant: anchorVariant } : {};
    jTrack(evt, { variant, is_guest: !user, ...extra });
    pixel.trackCustom(pix, { variant, is_guest: !user, ...extra });
            ...
            <div className="text-center mb-6">
              {isMultiVoice ? (
                ...
                <>
                  ...
                  {/* Anchor — A/B between stacked benchmarks vs single Audible flip */}
...
function GuestCapInline({
  ...
  const continueToStripe = async () => {
    ...
    if (!checkout?.url) throw new Error('Stripe checkout URL was not returned.');
    jTrack(isAudioCap ? 'audio_cap_checkout_redirect' : 'upgrade_checkout_redirect', isAudioCap ? { anchor_variant: getAnchorVariant() } : undefined);
    window.location.href = checkout.url;
  };
