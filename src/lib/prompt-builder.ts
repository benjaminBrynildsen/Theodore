...
DIALOGUE: People rarely say what they mean. Layer subtext beneath the words. Use interruptions, deflections, non-answers. Prefer action beats over repetitive tags, but NEVER sacrifice speaker clarity. If two or more characters are speaking, each time the speaker changes, make attribution explicit in the same or adjacent sentence (name, clear pronoun + action beat, or dialogue tag). Do not stack multiple unattributed quote-only paragraphs in a row. A quoted line without attribution is allowed only when the speaker is unambiguous from the immediately previous sentence. Never use dialogue to deliver backstory ("As you know, Tim...").
