...
export interface AutoGeneratedCanon {
  characters: {
    ...
    age?: string;          // 'Early 20s', '45', 'teenager', etc.
    pronouns?: string;     // 'he/him', 'she/her', 'they/them'
    // Concrete visual description that the image model can render. Example:
    // "8-year-old boy, olive skin, messy dark hair, round glasses, navy
    //  sweater over a white tee". Used to seed childrensBookSettings
    // .characterVisuals on creation so every page illustration has the
    // same character anchor instead of free-associating from prose.
