...
export function extractCanonFromConversation(messages: string[]): AutoGeneratedCanon {
  ...
  const STOP_NAMES = new Set([
    ...
    'Plan', 'Project', 'Create', 'Ready', 'Settings', 'Proposed', 'Conversation', 'Metadata', 'Title',
    'Length', 'Tone', 'Pacing', 'Character', 'Location', 'Artifact', 'Rule', 'Event', 'Systems',
    'Based', 'Current', 'Primary', 'Setting', 'Protagonist',
  ]);

  const explicitCharacterMatches = Array.from(
    text.matchAll(/\b(?:named|name is|called)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b/g),
  ).map((m) => m[1]);

  const aboutCharacterMatches = Array.from(
    text.matchAll(/\b(?:about|follows|following|with)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b/g),
  ).map((m) => m[1]);

  const roleCharacterMatches = Array.from(
    text.matchAll(/\b(?:protagonist|hero|heroine|villain|antagonist|mentor|detective|captain|king|queen)\s+(?:is|named|called)?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b/gi),
  ).map((m) => m[1]);

  const properNounCandidates = Array.from(text.matchAll(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b/g))
    .map((m) => m[1]);
  ...
  const filteredProperNouns = properNounCandidates.filter((name) => {
    if (name.length <= 2 || STOP_NAMES.has(name) || isTimeLike(name)) return false;
    if (name.includes(' ')) return true;
    return (nameFreq.get(name.toLowerCase()) || 0) > 1;
  });

  const characterMatches = [
    ...explicitCharacterMatches,
    ...aboutCharacterMatches,
    ...roleCharacterMatches,
    ...filteredProperNouns,
  ].filter((name) => !isTimeLike(name) && !STOP_NAMES.has(name));

  const locationMatches = [
    ...Array.from(text.matchAll(/\b(?:in|at|under|beneath|inside|from|to)\s+(?:the\s+)?([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){0,2})/g)).map((m) => m[1]),
    ...Array.from(text.matchAll(/\b(?:city|town|kingdom|realm|world|planet|station|district|valley|forest|island)\s+of\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){0,2})\b/gi)).map((m) => m[1]),
  ].filter((name) => !isTimeLike(name) && !STOP_NAMES.has(name));

  const systemMatches = [
    ...Array.from(text.matchAll(/\b([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){0,2})\s+(?:magic|system|protocol|order|law)\b/gi)).map((m) => `${m[1]} System`),
    ...Array.from(text.matchAll(/\b(?:magic|system|protocol|order|law)\s+(?:called|named)?\s*([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+){0,2})\b/gi)).map((m) => `${m[1]} System`),
  ].filter((name) => !isTimeLike(name));
